...
global.actionLibrary =
{
	...
	poison: {
		...
		targetAll: MODE.VARIES,
		userAnimation: "cast",
		effectSprite: sAttackBonk,  // Using existing animation
		effectOnTarget: MODE.ALWAYS,
		func: function(_user, _targets) {
			for (var i = 0; i < array_length(_targets); i++) {
				var target = _targets[i];
				// Initialize status effects array if it doesn't exist yet
				if (!variable_instance_exists(target, "statusEffects")) {
					target.statusEffects = [];
				}
				
				// Check if unit already has this effect
				var existingEffect = -1;
				for (var j = 0; j < array_length(target.statusEffects); j++) {
					if (target.statusEffects[j].name == "poison") {
						existingEffect = j;
						break;
					}
				}
				...
				if (existingEffect != -1) {
					target.statusEffects[existingEffect].duration = 3;
				} else {
					var newEffect = {
						name: "poison",
						duration: 3,
					...
					array_push(target.statusEffects, newEffect);
				}
				
				// Show status effect message
				instance_create_depth(target.x, target.y - 20, target.depth - 1, oBattleFloatingText, {
					text: "Poisoned!",
					color: c_green
				});
			}
			BattleChangeMP(_user, -global.actionLibrary.poison.mpCost);
		}
	},
	
	sleep: {
		name: "Sleep",
		description: "{0} casts Sleep!",
		subMenu: "Magic",
		mpCost: 10,
		targetRequired: true,
		targetEnemyByDefault: true,
		targetAll: MODE.VARIES,
		userAnimation: "cast",
		effectSprite: sAttackHeal,  // Using existing animation
		effectOnTarget: MODE.ALWAYS,
		func: function(_user, _targets) {
			for (var i = 0; i < array_length(_targets); i++) {
				var target = _targets[i];
				...
				var existingEffect = -1;
				for (var j = 0; j < array_length(target.statusEffects); j++) {
					if (target.statusEffects[j].name == "sleep") {
						existingEffect = j;
						break;
					}
				}
				...
				// If effect exists, refresh duration, otherwise add new effect
				if (existingEffect != -1) {
					target.statusEffects[existingEffect].duration = 2;
				} else {
					var newEffect = {
						name: "sleep",
						duration: 2,
	...
	paralysis: {
		...
		func: function(_user, _targets) {
			for (var i = 0; i < array_length(_targets); i++) {
				...
				var existingEffect = -1;
				for (var j = 0; j < array_length(target.statusEffects); j++) {
					if (target.statusEffects[j].name == "paralysis") {
						existingEffect = j;
						break;
					}
				}
			...
			BattleChangeMP(_user, -global.actionLibrary.paralysis.mpCost);
		}
	}
}
    ...
    [
        ...
        ,
        {
            ...
            actions : [global.actionLibrary.attack, global.actionLibrary.ice, global.actionLibrary.fire, 
                      global.actionLibrary.poison, global.actionLibrary.sleep, global.actionLibrary.paralysis]
        }
    ];
